...
**A shape is gathered to be chunked and not to be delivered, so the groups then come back in the album's own order, keyed on each group's oldest photo.** The order they come back in is the order they are uploaded in, and handing over every landscape and then every portrait runs the album's chronology twice. `L1 L2 P1 L3 P2 P3 L4` comes back as `(L1,L2) (P1,P2) (L3,L4) P3`. The key is the oldest member rather than the newest, which is what leaves a group where its first photo sat and keeps a photo added at the newest end from moving anything taken before it.
