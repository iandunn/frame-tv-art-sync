...
The play order is the strangest field in there, and it is the one that makes a rebuild survivable. The TV orders `play all` on insertion, so where a copy sits on the wall is decided by when it was uploaded relative to the others, which is as unchangeable after the fact as a matte and as invisible from the TV. Recording it turns "this copy is in the wrong place" into the same diff a matte change goes through, so switching `sync.play_order` replaces every image once and the run after that replaces none. That once matters, because uploading a couple of hundred images in a row is more than the Art app reliably survives: a run cut short leaves what it managed carrying the new order, so the next run uploads only the remainder rather than starting over, and it would die in the same place forever if it did. The remainder landing last is right rather than lucky, since a reversed handover was going to put the oldest photos at the end anyway. What none of it survives is a photo being added, because nothing can be inserted ahead of an image already on the TV, so a new photo plays first until `frame sync --force` rebuilds.

An entry written before the field existed reads as `newest_first` rather than as unknown, which is the one stand-in in `render._DEFAULTS` that isn't a value nothing matches. It is a fact rather than a guess: every upload before the key existed went up in album order, which is what the name means. Reading it as unknown would rebuild the album for a config that never asked for anything.

The date is the one field in there that isn't about the pixels either. It is `image_date`, the only text an upload carries, and it is recorded for the reason everything else is: it can only be set as an image is uploaded, and `available()` reports it back empty. A sync sends the moment the group's oldest photo was taken, in UTC, rather than letting `samsungtvws` fill in the clock, which would date every image on the wall to the run that put it there. UTC because a source reports an instant and not the offset it was captured at, and a stamp that moved with the running machine's timezone would replace the album the first time a scheduled job ran from somewhere else.
...
An entry with no `render` table is one written before records existed, and so is every entry in a file whose `version` is below the current one, since a record from then describes a shape today's code has nothing to compare against. Either way it reads as unknown, and unknown loses: those photos are uploaded again. A record missing one *field* is the narrower case of a field added after it was written, which `render._DEFAULTS` names: the field reads as a stand-in and the stand-in is never written back to the file. What the stand-in is depends on whether the older upload's value is knowable. It isn't for `image_date`, which reads as a value nothing matches so the image is made again; it is for `play_order`, where every upload before the key existed went up in album order. Every other field stays required, because guessing at one of those decides whether an image is destroyed. A `render` table that is present but unreadable is an error rather than an unknown, because the alternative is guessing at a value that decides whether an image is destroyed.
